...
impl CoalescingPersister {
    pub fn new(history: Arc<dyn ChatHistory>, chat_id: String) -> Self {
    ...
    pub fn schedule(&self, messages: Vec<Message>) {
    ...
    pub async fn checkpoint(&self, messages: Vec<Message>) {
...
mod tests {
    ...
    async fn rapid_schedules_coalesce_to_latest_wins() {
        ...
        assert!(matches!(
            &last[19].content[0],
            dyson_core::ContentBlock::Text { text } if text == "msg 19"
        ));
    }
