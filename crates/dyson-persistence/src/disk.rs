...
use crate::resolve_tilde;
use dyson_core::Result;
use dyson_core::{ContentBlock, Message};
...
impl DiskChatHistory {
    ...
    pub fn new(dir: PathBuf) -> Result<Self> {
        ...
        if let Err(e) = crate::migrate::migrate(&dir) {
...
mod tests {
    ...
    fn save_and_load() {
        ...
        let messages = vec![
            Message::user("hello"),
            Message::assistant(vec![dyson_core::ContentBlock::Text {
                text: "hi there".into(),
            }]),
        ];
        ...
        assert_eq!(loaded[0].role, dyson_core::Role::User);
        assert_eq!(loaded[1].role, dyson_core::Role::Assistant);
    ...
    fn failed_write_leaves_original_transcript_intact() {
        ...
        let result = write_atomically(&path, |w| {
            ...
            Err(dyson_core::DysonError::Llm("simulated crash".into()))
        });
    ...
    fn new_save_after_rotate() {
        ...
        match &loaded[0].content[0] {
            dyson_core::ContentBlock::Text { text } => assert_eq!(text, "second"),
