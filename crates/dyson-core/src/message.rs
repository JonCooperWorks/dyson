...
pub enum ArtefactKind {
    ...
    SecurityReview,
    /// An eval run report emitted by the swarm's `evals.run` MCP tool.
    /// Carries `metadata.report_kind = "eval"` so the reader renders it with
    /// the shared eval-results view rather than the security-report view.
    EvalReport,
    /// An image generated by a tool (e.g. `image_generate`).  The
    /// `content` field holds the served URL (`/api/files/<id>`) and
    /// `mime_type` is the image MIME — the reader renders it with
    /// `<img>` rather than the markdown pipeline.
    Image,
