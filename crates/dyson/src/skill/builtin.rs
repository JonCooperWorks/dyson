...
impl BuiltinSkill {
    ...
    pub fn new_filtered(
        ...
        if let Some(ig_cfg) = image_provider_config {
            match image_generate::create_provider(ig_cfg) {
                Ok(provider) => {
...
impl BuiltinSkill {
    ...
    pub fn new(
        ...
        Self::new_filtered(web_search_config, image_provider_config, &[])
    }
}
...
mod tests {
    ...
    fn has_builtin_tools() {
        let skill = BuiltinSkill::new(None, None);
    ...
    fn empty_filter_includes_all() {
        let all = BuiltinSkill::new(None, None);
        let filtered = BuiltinSkill::new_filtered(None, None, &[]);
    ...
    fn image_generate_tool_registered_when_configured() {
        ...
        let skill = BuiltinSkill::new(None, Some(&config));
        let names: Vec<&str> = skill.tools().iter().map(|t| t.name()).collect();
