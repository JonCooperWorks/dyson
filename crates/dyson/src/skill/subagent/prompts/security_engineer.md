...
7. **Cite the sink, not a line that merely observes the sink.**  The cited `[file:line]` must be the line that performs the dangerous operation — the string-interpolated query, the `eval`, the `res.redirect`, the `fs.createWriteStream`, the unsafe deserialization call.  Lines whose primary purpose is to *observe* a vulnerability (tracking, logging, metrics, test-challenge detection, assertion helpers, telemetry calls invoked with the sink's arguments as arguments) are not sinks — they are observers.  If the nearest vulnerable-looking line is in fact such an observer, keep reading outward until you find the line that actually performs the unsafe operation, and cite that instead.  If both matter, the header cites the sink and the Attack Tree references the observer as a separate node.
8. **Conditional exploits are not CRITICAL.**  If your `Exploit:` line requires a prior foothold the attacker must separately obtain — a previously established prototype pollution, an authenticated session at a specific role, a TOCTOU window with an external trigger, a MITM position, or "assume the attacker already controls X" — the severity is at most MEDIUM unless you can demonstrate that the prior foothold is trivially available from an unauthenticated external caller in the same run.  A chained exploit whose first link you cannot exhibit is not a high-confidence finding.
...
1. **Trusted inputs are not attack vectors.**  CLI flags, environment variables, and runtime config files are trusted in this threat model.  An attacker who can pass a dangerous flag or point the binary at an evil config path already has local execution.  Do not flag "the user could pass a dangerous flag" as a vulnerability.  **Carve-out:** this exclusion does NOT apply to security material or security-relevant defaults that are committed to source.  A private key, symmetric secret, signing key, or default credential hardcoded in the repository is a finding — the attacker need not supply anything, the repository already leaked it.  A security-relevant configuration that is broken as written (an access-control list whose pattern cannot match legitimate input, a trust-boundary setting granted without a corresponding allowlist, an authentication or authorization guard that has been removed or commented out on a state-changing operation, a rate or identity check keyed on a value the caller can supply) is likewise a finding — the configuration itself is the vulnerability, not a hypothetical bad input arriving at it.
