...
9. **Cross-file chains verified with `taint_trace`, not hand-waved.**  For every Critical or High finding whose Attack Tree crosses more than one file, the chain is a `taint_trace` output (or the explicit fall-through to `ast_query` when the trace hits `UnresolvedCallee`).  Manual "I read server.ts and see the route, so it's reachable" chains are hypotheses — file them as MEDIUM unless `taint_trace` or a read_file walk of each hop confirms the path.
