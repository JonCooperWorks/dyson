...
// severity/affected-ranges/references. Applications should inject their
// shared policy-configured client with [`OsvClient::with_http`].
...
impl OsvClient {
    pub fn new() -> Self {
        Self::new_with(DEFAULT_BASE.to_string(), reqwest::Client::new())
    }

    /// Use an application-owned HTTP client while retaining the canonical
    /// OSV endpoint. This preserves connection pooling and outbound policy.
    pub fn with_http(http: reqwest::Client) -> Self {
        Self::new_with(DEFAULT_BASE.to_string(), http)
    }
