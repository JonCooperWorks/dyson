...
| **Partial hit** (adjacent real finding, not the exact CVE) | 6 | spring-beans (ClassEditor vs Spring4Shell), mastodon (LinkDetails iframe vs formatter), keycloak (testSMTPConnection + stack trace leak), airflow (4 webserver-hardening findings), bookstack (hardcoded APP_KEY + SAML2 CSRF-exempt), plausible (open redirect on logout) |
...
| [`iter11-plausible-2.0.0-partial-hit.md`](iter11-plausible-2.0.0-partial-hit.md) | Plausible Analytics v2.0.0 `lib/plausible_web` | Elixir/Phoenix | One real finding: open redirect via `redirect` query parameter on `/logout` at `auth_controller.ex:556` (`redirect(to: Map.get(params, "redirect", "/"))` — host-controlled target).  Clean Elixir/Phoenix attack-tree analysis with idiomatic `URI.parse` remediation snippet.  No staged CVE — the target was set up as a first-Elixir framework exercise.  First Elixir / Phoenix target — exercised `lang/elixir.md` + `framework/phoenix.md`. |
