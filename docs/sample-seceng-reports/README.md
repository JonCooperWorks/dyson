...
| [`iter7-pyyaml-5.3-hit.md`](iter7-pyyaml-5.3-hit.md) | PyYAML 5.3 | **Hit** (comprehensive) | Five findings covering the full FullLoader / UnsafeLoader RCE surface: `!!python/object/apply:`, `!!python/object/new:` (the CVE-2020-1747 path), `!!python/object:` state injection via `__setstate__`, `!!python/name:` function references, and `unsafe_load` alias.  13.5KB.  Shows comprehensive library-level review (multiple tag-type sinks enumerated) rather than pinning a single CVE. |
...
| [`iter8-strapi-4.4.5-hit.md`](iter8-strapi-4.4.5-hit.md) | Strapi 4.4.5 admin server | **Hit** (multiple) | Five findings including **CVE-2023-22894 SSRF via admin webhook** (CRITICAL), JWT signature algorithm bypass (CRITICAL), password reset token in URL query string, and admin static-files auth gap.  5 real taint_trace calls, 1 inlined.  Clean rediscovery on a new framework (Koa) the suite hadn't exercised before. |
| [`iter8-mastodon-4.0.2-partial-hit.md`](iter8-mastodon-4.0.2-partial-hit.md) | Mastodon 4.0.2 `app/lib` | **Partial hit** | Did NOT pin CVE-2023-36462 (HTML injection in toot formatter) exactly.  Instead filed five adjacent findings: stored HTML injection via `LinkDetailsExtractor` iframe (same sanitiser-gap class, different subsystem), WebFinger host-meta SSRF, admin-only SQL string interpolation, unscoped `send(key)` via ActiveModel serialisation, Nokogiri with known libxml2 CVEs.  13.4KB, 2 real taint_trace, 1 inlined.  First Ruby target in the sample set. |
