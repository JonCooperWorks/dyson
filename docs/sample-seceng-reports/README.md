...
|---|---|---|
| **Hit** (CVE pinned OR multiple real findings) | 14 | log4j, jackson, nextjs, pyyaml, lodash, commons-text, webgoat, ghost, strapi, react-server, tomcat, node-forge, meilisearch, rocketchat |
...
Stacks exercised: **Java** (Log4j, Spring, jackson-databind, Commons Text, WebGoat, Tomcat, Keycloak), **JavaScript/TypeScript** (lodash, Next.js, React server + adapter, Ghost, Strapi, node-forge, ejs, Rocket.Chat), **Python** (PyYAML, Django, Airflow), **Ruby** (Mastodon), **Go** (Gitea, Grafana), **Rust** (Meilisearch), **PHP** (BookStack), **Elixir** (Plausible).

## Verification pass (2026-04-20)

Manual re-audit of every CRITICAL / HIGH / MEDIUM finding across iter7–iter12, cross-checking each report against its source tree and every cited CVE/GHSA against the public advisory.
