...
|---|---|---|
| **Hit** (CVE pinned OR multiple real findings) | 16 | log4j, jackson, nextjs, pyyaml, lodash, commons-text, webgoat, ghost, strapi, react-server, tomcat, node-forge, meilisearch, rocketchat, wormhole-solana, sealevel-attacks |
| **Partial hit** (adjacent real finding, not the exact CVE) | 6 | spring-beans (ClassEditor vs Spring4Shell), mastodon (LinkDetails iframe vs formatter), keycloak (testSMTPConnection + stack trace leak), airflow (4 webserver-hardening findings), bookstack (hardcoded APP_KEY + SAML2 CSRF-exempt), cashio post-tune (HIGH on the unanchored-chain bug, should have been CRITICAL) |
| **Novel-target runs** (latest-stable, not pinned to known CVE) | 2 | outline (6 dep CVEs mapped to reachability), appwrite (OAuth redirect XSS real + PHP proto-walk FP + 2 minor) |
| **Near-miss** (right code examined, safe-looking reasoning, no finding) | 3 | django (Trunc/Extract regex rationale), spring-security (RegexRequestMatcher dismissed), plausible (only finding is a dud — Phoenix `redirect(to:)` is local-only) |
| **Miss** | 3 | react-server-dom-webpack (dismissed as delegator), gitea (filed HTML-attr MEDIUM, missed SVG SSRF), cashio pre-tune (walked past the $52M Cashio bug — fixed by adding the unanchored-chain class to `solana.md`) |

Stacks exercised: **Java** (Log4j, Spring, jackson-databind, Commons Text, WebGoat, Tomcat, Keycloak), **JavaScript/TypeScript** (lodash, Next.js, React server + adapter, Ghost, Strapi, node-forge, ejs, Rocket.Chat), **Python** (PyYAML, Django, Airflow), **Ruby** (Mastodon), **Go** (Gitea, Grafana), **Rust** (Meilisearch), **Solana / Anchor on-chain** (Cashio, Wormhole-Solana, sealevel-attacks), **PHP** (BookStack), **Elixir** (Plausible).

## Verification pass (2026-04-20)

Manual re-audit of every CRITICAL / HIGH / MEDIUM finding across iter7–iter12, cross-checking each report against its source tree and every cited CVE/GHSA against the public advisory.  iter13 (Solana batch) was added the same day post-verification; spot-checked against the Cashio post-mortem and the Wormhole sysvar-substitution disclosure, both rediscoveries are clean.

**Fabricated identifiers found: zero.** Every CVE / GHSA / RUSTSEC ID spot-checked resolves to a real advisory for the cited package and version.  The one 404 (`GHSA-h395-gr665-qw3r` on meilisearch) points to a real underlying vuln (CVE-2024-37963 in `jsonwebtoken <10.3.0`) under a different advisory ID — class correct, label wrong.

**File:line citations** match the source byte-for-byte in every report checked.  No hallucinated locations.

**Clean rediscoveries** — code, impact, and CVE ID all check out: log4j (CVE-2021-44228), jackson-databind (4 findings), pyyaml (5 findings, incl. CVE-2020-1747), lodash 4.17.11 (CVE-2019-10744 + defaultsDeep), commons-text (CVE-2022-42889), tomcat (CVE-2020-1938 + 4 adjacent), strapi (CVE-2023-22894 + alg=none), next.js (CVE-2025-29927), outline dep audit (sequelize 6.37.7 → GHSA-6457-6jrx-69cr confirmed; koa 3.1.1 still vulnerable to GHSA-7gcc-r8m5-44qm; oauth2-server 5.2.1 still vulnerable to GHSA-jhm7-29pj-4xvf; lodash 4.17.21 → GHSA-f23m-r3pf-42rh real), bookstack (both findings real).

**Severity inflation** — code real, rating too high:

- [`iter8-react-server-19.2.0-hit.md`](iter8-react-server-19.2.0-hit.md) — four "CRITICAL" findings are latent primitives neutralised by downstream validators; body text admits this.  Should be HIGH / hardening.
- [`iter8-ghost-5.59.0-hit.md`](iter8-ghost-5.59.0-hit.md) — "SQL Injection" is bookshelf-bounded NQL filter injection; the "no-auth upload" CRITICALs treat endpoint-layer `permissions: false` as full unauth without checking router middleware.
- [`iter9-spring-security-5.6.2-near-miss.md`](iter9-spring-security-5.6.2-near-miss.md) — HIGH on a `logger.debug()` URL log-injection is clearly LOW / informational.  Report also has a malformed `# CRITICAL` header with only MEDIUM findings under it.
- [`iter7-spring-beans-partial-hit.md`](iter7-spring-beans-partial-hit.md) — ClassEditor HIGH needs a downstream sink to reach RCE; YamlProcessor MEDIUM is neutralised by the empty `supportedTypes` default (report self-flags).
- [`iter9-node-forge-1.2.1-hit.md`](iter9-node-forge-1.2.1-hit.md) — TLS 1.2 PRF stub HIGH is unreachable (only TLS 1.0 / 1.1 in `SupportedVersions`); e=3 MEDIUM is spec-compliant.
- [`iter10-meilisearch-1.4.0-hit.md`](iter10-meilisearch-1.4.0-hit.md) — the JWT type-confusion CVE-2024-37963 only bypasses `exp` / `nbf` time checks; the report extrapolates to forged `search_rules` / `api_key_uid`, which the advisory doesn't support.

**Factually wrong** — code cited, reasoning doesn't hold:

- [`iter11-plausible-2.0.0-partial-hit.md`](iter11-plausible-2.0.0-partial-hit.md) — Phoenix `redirect(to: ...)` validates local-URL-only and raises on a full URL.  The only finding in the report is a dud.  Reclassify from "partial hit" to "near-miss".
- [`iter12-appwrite-1.9.0-novel-target.md`](iter12-appwrite-1.9.0-novel-target.md) — the `parseMultipart` "prototype-walk primitive" is a JS-ism; PHP associative arrays have no prototype chain so `$items['__proto__']` is just a string key.  Report caps at LOW and self-flags, so the rating is defensible but the framing is wrong.

**Confirmed misses** (correct "no findings"): [`iter9-grafana-9.3.6-miss.md`](iter9-grafana-9.3.6-miss.md).  [`iter8-gitea-1.17.3-miss.md`](iter8-gitea-1.17.3-miss.md) is technically a weak MEDIUM that bluemonday's `SanitizeReader` neutralises — file is named "miss" which is defensible.
...
| [`iter11-rocketchat-6.0.0-hit.md`](iter11-rocketchat-6.0.0-hit.md) | Rocket.Chat 6.0.0 `apps/meteor/server/methods` | Node / Meteor | Six findings on Meteor DDP method authorization: unauthorized file deletion via `deleteFileMessage`, MongoDB regex-injection (ReDoS) in `messageSearch`, three missing-authentication methods (`loadLocale`, `getSetupWizardParameters`, `logoutCleanUp`), missing permission check on `getRoomById`.  First Meteor framework exercise in the suite — exercised `framework/meteor.md`.  Did not pin CVE-2023-28359 subscription leak specifically, but the Meteor method-authz class is the same bug family. |
| [`iter13-wormhole-solana-hit.md`](iter13-wormhole-solana-hit.md) | Wormhole bridge program `solana/bridge/program` (pinned to `79ab522`, pre-patch) | Rust / Solana | **CRITICAL sysvar-substitution bypass at `verify_signature.rs:34`** — `instruction_acc: Info<'b>` with no `key == sysvar::instructions::id()` check, allowing an attacker-controlled fake account at the sysvar slot to fabricate a `secp256k1_program` instruction and bypass all guardian-signature verification.  Full attack tree, typed-`Sysvar` remediation.  Plus a second CRITICAL on borsh 0.8.1 ZST unsoundness (RUSTSEC-2023-0033, real CVE) and a HIGH on `try_borrow_mut_data()` against a read-only sysvar account (separate real defect).  First Solana / Anchor target in the suite — exercised the new `framework/solana.md` cheatsheet's "Sysvar substitution" rule first try.  Correctly omits `Taint Trace:` per the sheet's "missing-predicate vulns ⇒ omit trace, paste constraint snippet" guidance. |
| [`iter13-sealevel-attacks-hit.md`](iter13-sealevel-attacks-hit.md) | `coral-xyz/sealevel-attacks/programs` (deliberately-vulnerable teaching corpus) | Rust / Anchor | **Every `insecure/` variant flagged**: 6 CRITICAL (signer-authorization, type-cosplay, arbitrary-cpi, reinitialization, pda-sharing, bump-canonicalization) + 5 HIGH (closing-accounts, sysvar-address-checking, owner-checks, account-data-matching, duplicate-mutable-accounts) + 1 MEDIUM (close-without-discriminator).  Every `secure/` variant correctly listed in `Checked and Cleared`.  Reference example of the full `solana.md` cheatsheet exercised against the canonical Solana attack-class taxonomy.  Minor preamble leak (`Now I have all the data I need…`) at the top. |
...
| [`iter12-outline-1.6.1-dep-audit.md`](iter12-outline-1.6.1-dep-audit.md) | Outline 1.6.1 `server/routes/api` | TS/Koa | **Novel-target run** (latest stable, no pinned CVE).  Six dep-driven findings mapped to reachable in-scope code paths: Sequelize 6.37.7 SQLi via JSON column cast type (GHSA-6457-6jrx-69cr) traced to `documents.ts:146-168` user-controlled `where` clauses, Koa `ctx.hostname` header-trust (GHSA-7gcc-r8m5-44qm), PKCE code-verifier bypass in OAuth2 (GHSA-jhm7-29pj-4xvf), lodash proto-pollution, markdown-it ReDoS, MCP SDK cross-client data leak.  4 real taint_trace calls, 0 fabrication.  Agent correctly chose **dep-audit + reachability** over inventing code-level findings on a well-reviewed codebase — the right behavior.  Actionable: bumping sequelize 6.37.7 → 6.37.8 closes the SQLi. |
| [`iter12-appwrite-1.9.0-novel-target.md`](iter12-appwrite-1.9.0-novel-target.md) | Appwrite 1.9.0 `app/controllers/api` | PHP BaaS | **Novel-target run** (latest stable, no pinned CVE).  Four code-level findings, mixed quality:  **1 plausibly disclosable**: OAuth2 failure-redirect reflects attacker-controlled `error_description` into the `error` query param without HTML-escape (`account.php:1591`) — conditional on downstream renderer but defense-in-depth fix is warranted.  **1 likely FP**: GraphQL multipart property-walk at `graphql.php:300` — agent applied the JS prototype-walk rule to PHP where `$items['__proto__']` is just a string key, not a reflection primitive.  Agent correctly flagged the coincidental-guard caveat and capped at MEDIUM.  **Plus 2 minor hardening**: user-configurable email templates without render restrictions, `$clientName` from `UserAgent` into email template.  11.7KB, 0 taint_trace, 0 fabrication, preamble leak.  Instructive as a cross-language-cheatsheet-leak example: the JS proto-walk pattern over-applied to PHP. |
| [`iter13-cashio-post-tune-partial-hit.md`](iter13-cashio-post-tune-partial-hit.md) | Cashio `programs/brrr` (pinned to `a51c3c5`, last pre-hack commit) | Rust / Anchor / Solana | **Found the $52M Cashio bug as HIGH instead of CRITICAL.**  After `solana.md` got the new "Unanchored validation chain (typed accounts ≠ canonical accounts)" class, the agent correctly flagged `BrrrCommon::validate()` at `actions/mod.rs:10-22`, enumerated all 6 `assert_keys_eq!` calls as relative-only, named the Cashio vulnerability pattern by date and dollar figure, and proposed the right anchored-comparison fix.  Severity hedged to HIGH because the agent reasoned the downstream `crate_token` program *might* re-validate authority — same logic that let the bug ship in 2022.  Severity-default wording in the cheatsheet has since been tightened ("do not downgrade because the downstream program is out of scope"); iter3 verification rerun in flight.  Bonus MEDIUM correctly identifies `hard_cap` as a secondary vector via the same root cause.  Pair with [`iter13-cashio-pre-tune-miss.md`](iter13-cashio-pre-tune-miss.md) for the run → grade → tune → rerun loop. |
...
| [`iter11-plausible-2.0.0-partial-hit.md`](iter11-plausible-2.0.0-partial-hit.md) | Plausible Analytics v2.0.0 `lib/plausible_web` | Elixir/Phoenix | Filed one LOW as an open redirect via `redirect` query parameter on `/logout` at `auth_controller.ex:556`.  On re-audit this is a non-finding: Phoenix's `redirect(to: path)` validates local-URL-only and raises `ArgumentError` on any full URL — the claimed exploit cannot occur.  Reclassified from partial-hit to near-miss: right code, wrong conclusion about the framework's default-safe behavior.  Filename kept for git history; see the Verification pass note above. |
...
| [`iter9-grafana-9.3.6-miss.md`](iter9-grafana-9.3.6-miss.md) | Grafana 9.3.6 `pkg/services/dashboards` | Go/Chi | CVE-2023-0594 (stored XSS in dashboard panel metadata).  Filed "No exploitable findings meet the Finding Gate criteria within this codebase scope."  2 real taint_trace, 0 fabrication.  Likely a scope mismatch — `pkg/services/dashboards` is the DB-access service layer; stored-XSS render usually lives in `pkg/api` or the frontend render path.  Useful case study in "scope selection is load-bearing". |
| [`iter13-cashio-pre-tune-miss.md`](iter13-cashio-pre-tune-miss.md) | Cashio `programs/brrr` (pinned to `a51c3c5`, last pre-hack commit) | Rust / Anchor / Solana | **Walked past the $52M Cashio infinite-mint bug.**  Filed 0 findings, then quoted the vulnerable line (`bank == collateral.bank — bank integrity` at report line 26) as proof of safety.  The pre-tune `solana.md` covered "Missing owner check" only at the `AccountInfo` / `UncheckedAccount` level — it taught the agent that `Account<'info, T>` was safe because Anchor checks the owner program.  True for *program-ownership*; missed *canonical-instance trust*.  Bankman is permissionless: anyone can mint a fresh `Bank` with attacker-chosen fields, then thread a fully self-consistent fake chain through the validator.  Reference example of a cheatsheet gap that motivated a targeted addition (the new "Unanchored validation chain" class), verified by [`iter13-cashio-post-tune-partial-hit.md`](iter13-cashio-post-tune-partial-hit.md).  Useful case study in "the run → grade → tune → rerun loop the README's parent doc describes". |
...
2. **Preamble shape** — no report's first non-whitespace character should be anything but `#`.  Strings like `Now I have enough evidence…`, `Let me compile…`, `Based on my analysis…` at the top are the forbidden preamble class.  Several reports here still open with one (notably iter7 jackson, iter8 commons-text, iter8 webgoat, iter13 sealevel-attacks); the findings were real so they shipped, but the preambles are a persistent weaker-model tell worth keeping an eye on.
